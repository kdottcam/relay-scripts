...
if not name then
    return warn(
        ("[Relay] Unsupported game (GameId: %d, PlaceId: %d)"):format(
            game.GameId,
            game.PlaceId
        )
    )
...
if not ok then
    return warn("[Relay] Failed to download " .. name .. ": " .. tostring(source))
end

local compiled, compileError = loadstring(source)

if not compiled then
    return warn("[Relay] Failed to compile " .. name .. ": " .. tostring(compileError))
end

local ran, runtimeError = pcall(compiled)

if not ran then
    warn("[Relay] Runtime error in " .. name .. ": " .. tostring(runtimeError))
end
